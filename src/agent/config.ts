...
export function validateConfig(config: AgentConfig, requireModel = true): AgentConfig {
  ...
  const integers: [number, number, number, string][] = [
    [result.contextWindow, 4096, Number.MAX_SAFE_INTEGER, "上下文窗口"],
    [result.maxOutputTokens, 128, Number.MAX_SAFE_INTEGER, "最大输出"],
    [result.inputBudget, 0, Number.MAX_SAFE_INTEGER, "单次输入预算"],
    [result.totalInputBudget, 0, Number.MAX_SAFE_INTEGER, "单问累计输入预算"],
  ...
  if ((result.inputBudget > 0 && result.inputBudget < 8000) || (result.totalInputBudget > 0 && result.totalInputBudget < 16000)) throw new Error("输入预算填 0 表示不设额外上限；启用时单次至少 8,000、累计至少 16,000。");
  if (result.totalInputBudget > 0 && result.inputBudget > 0 && result.totalInputBudget < result.inputBudget * 2) throw new Error("累计输入预算至少为单次预算的两倍，以预留总结空间。");
