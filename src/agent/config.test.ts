...
describe("Agent API configuration", () => {
  it("restores older settings without overwriting preferences and resolves all reading modes", () => {
    expect(validateConfig(DEFAULT_AGENT_CONFIG)).toMatchObject({ readingDepth: "auto", maxSteps: 0 });
    const legacy = { inputBudget: 24000, maxOutputTokens: 512, contextWindow: 8192, outputParameter: "max_tokens", instructions: "", apiKey: "retained-test-key" };
    expect(restoreConfig(legacy)).toMatchObject({ instructions: "", apiKey: "retained-test-key", customReadingChars: 4500 });
    for (const key of ["inputBudget", "totalInputBudget", "contextWindow", "maxOutputTokens", "outputParameter"]) expect(restoreConfig(legacy)).not.toHaveProperty(key);
  ...
  it("validates remaining operational settings", () => {
