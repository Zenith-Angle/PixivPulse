...
export const NOVEL_TOOL: ToolDefinition = {
  name: "sample_novel", description: "Read limited visible original novel excerpts (not title/description). Bounded excerpts with exact positions; application uses progressive per-work sampling; continue with unread positions as needed. Prefer balanced for initial multi-position reading. Repeat to obtain unread positions; refresh is NOT needed for continuation. Only known local novels. Report actual coverage; never infer unseen plot. Use opening/middle/ending/balanced or keyword; keyword empty unless keyword focus. Refresh false reuses 24h cache.",
