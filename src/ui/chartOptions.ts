...
type SliderVisibility = "auto" | "always";

const chartNavigation = (pointCount: number, sliderVisibility: SliderVisibility = "auto") => {
  const showSlider = sliderVisibility === "always" || pointCount > 24;
  return {
    grid: { ...baseGrid, bottom: showSlider ? 64 : baseGrid.bottom },
    dataZoom: [
      {
...
export const buildPortfolioChartOption = ({
  ...
  return {
    ...
    ...chartNavigation(validPoints.length),
    tooltip: {
      trigger: "axis",
      axisPointer: {
        type: "line",
...
export const buildFollowerChartOption = ({
  ...
  return {
    ...
    textStyle: baseTextStyle,
    ...(compact
      ? { grid: { left: 2, right: 2, top: 5, bottom: 5, containLabel: false }, dataZoom: undefined }
      : chartNavigation(validPoints.length)),
...
export const buildAbsoluteCompareChartOption = ({
  ...
  return {
    ...
    aria: { enabled: true },
    textStyle: baseTextStyle,
    ...chartNavigation(maxPointCount, "always"),
    legend: {
      show: false,
...
export const buildGrowthChartOption = ({
  ...
  return {
    ...
    aria: { enabled: true },
    textStyle: baseTextStyle,
    ...chartNavigation(normalized.length),
    tooltip: {
      trigger: "axis",
      axisPointer: {
        type: "line",
