...
export const formatElapsed = (hours: number | null): string => {
  if (hours === null || !Number.isFinite(hours)) return t("暂无有效间隔");
  if (hours < 1) {
    const minutes = Math.max(1, Math.round(hours * 60));
    return t("距上次采样 {value0} 分钟", { value0: minutes });
  }
  if (hours < 24) return t("距上次采样 {value0} 小时", { value0: hours.toFixed(1) });
  return t("距上次采样 {value0} 天", { value0: (hours / 24).toFixed(1) });
};

export const formatTimestamp = (value: string | null | undefined): string => {
  const timestamp = toTime(value);
  if (timestamp === null) return t("未记录");
...
export const toSafeCsv = (data: DashboardData): string => {
  const rows: unknown[][] = [
    [t("作品 ID"), t("标题"), t("类型"), t("浏览"), t("赞"), t("收藏"), t("评论"), t("上次变化 UTC"), t("上次变化（北京时间 UTC+8）"), t("首次观察 UTC"), t("首次观察（北京时间 UTC+8）")],
    ...data.works.map((work) => [
      work.id,
      work.title,
      work.type === "illust" ? t("插画") : t("小说"),
      work.metrics.views,
      work.metrics.likes,
...
export const confidenceLabel = (confidence: WorkAnalysis["confidence"]): string => {
  if (confidence === "high") return t("高置信");
  if (confidence === "medium") return t("中置信");
  return t("样本不足");
};
