...
Chrome 商店更新包：`release/pixiv-pulse-v0.5.17-chrome.zip`。商店文案：[中文版](./docs/store-listing.zh-CN.md) · [English](./docs/store-listing.en.md)。

## 安装与升级

当前源码版本：**v0.5.17**（本地构建，尚未发布）。需要 **Chrome 120+**，通过已解压扩展安装。

### 首次安装

1. 在 [最新 Release](https://github.com/Zenith-Angle/PixivPulse/releases/latest) 下载对应已发布版本的 Chrome ZIP；本地构建 0.5.17 可使用 `.output/pixiv-pulse-0.5.17-chrome.zip`。请选择这个安装包，GitHub 自动生成的 **Source code** 是源码包。
...
```powershell
Get-FileHash .\pixiv-pulse-v0.5.17-chrome.zip -Algorithm SHA256
```
...
商店上传包不包含本地开发公钥。Chrome 商店用户通过原商店条目更新；此前使用固定开发 ID 的本地安装，请运行 `npm run build:installed` 并继续使用 `.extension-dev/chrome-mv3`，不要用商店 ZIP 覆盖该开发目录。
