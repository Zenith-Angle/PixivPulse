...
作品卡片与列表可直接查看今日浏览增量，点击小图打开详情。详情与总览同时展示累计曲线和分段增量，指标、时间范围共用；长范围平滑限制在典型采样间隔内，保留阶段性加速、减速。

比较选择集中在比较页：可搜索标题、ID、系列，按类型筛选，只看已选，批量加入（最多 5 件）、逐件移除或清空。作品页不再显示比较复选框。

增量按每段内观察到的净变化计算，变化计入观察结束所在时段；没有观察的时段留空，负值保留。跨越长间隔的变化不代表全部发生在最后时段；作品集总量差额也可能包含新纳入作品的初始总量。
